...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, kv=None, key_padding_mask=None, return_attention=False):
        ...
        q = self.q(q) # Query remains same # Shape [batch, query_seq_len, hidden]
        k = self.k(q) if kv is None else self.k(kv) # Shape [batch, query_seq_len, hidden]; if kv supplied [batch, kv_seq_len, hidden]
        v = self.v(q) if kv is None else self.v(kv) # Shape [batch, query_seq_len, hidden]; if kv supplied [batch, kv_seq_len, hidden]
        # YOUR CODE ENDS HERE
        ...
        if key_padding_mask is not None:
            ...
            # Just trying to reshape such that the shape ends with [*, *, batch, attended_seq] since the mast will be straight applied repeatedly to the last two dimensions
            scores = scores.reshape(bs, self.num_heads, attending_seq, attended_seq).permute(1, 2, 0, 3) # shape [num_heads, attending_seq, batch, attended_seq]
            scores = scores.masked_fill(key_padding_mask == 1, -float("Inf")) # shape [num_heads, attending_seq, batch, attended_seq]
            scores = scores.permute(2, 0, 1, 3).reshape(bs * self.num_heads, attending_seq, attended_seq) # Reversing the above reshape: shape [batch * num_heads, attending_seq, attended_seq]
