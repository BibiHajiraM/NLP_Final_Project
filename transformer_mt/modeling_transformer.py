...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, hidden, num_heads, fcn_hidden, dropout=0.0):
        ...
        self.self_attention = MultiHeadAttention(input_size=hidden, hidden=hidden, num_heads=num_heads, causal=False) # Causal not needed for translation; need to consider all tokens
        self.cross_attention = MultiHeadAttention(input_size=hidden, hidden=hidden, num_heads=num_heads, causal=True) # Need to mask future tokens as translated text is generated "sequentially"

        self.self_att_layer_norm = nn.LayerNorm(hidden, eps=1e-6) # Taking this from my previous homework
        self.cross_att_layer_norm = nn.LayerNorm(hidden, eps=1e-6)

        # This block is taken from my previous homework as well
        self.fcn = nn.Sequential(
          nn.Linear(hidden, fcn_hidden),
          nn.ReLU(),
          nn.Linear(fcn_hidden, hidden),
        ...
        self.fcn_layer_norm = nn.LayerNorm(hidden, eps=1e-6)
        self.dropout = nn.Dropout(dropout)
        # YOUR CODE ENDS HERE 
    ...
    def forward(self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None):
        ...
        # NOTE: Code mostly taken from previous homework

        # Self-attention
        attention_flow = self.self_attention.forward(q=decoder_hidden_states, return_attention=True) # [batch_size, query_seq_len, hidden]
        attention_flow = attention_flow[0] + decoder_hidden_states # Skip connection (residual): Size [batch_size, query_seq_len, hidden]
        attention_flow = self.self_att_layer_norm(attention_flow) # Just normalization so shape will remain the same: [batch_size, query_seq_len, hidden]
        # Cross-attention
        # Padding mask needed here to avoid use of unwanted tokens (from encoder) 
        attention_flow = self.cross_attention.forward(q=decoder_hidden_states, kv=encoder_hidden_states, key_padding_mask=key_padding_mask, return_attention=True) # [batch_size, query_seq_len, hidden] Because in cross attention q has query_seq_len 
        attention_flow = attention_flow[0] + decoder_hidden_states # Skip connection (residual): Size [batch_size, query_seq_len, hidden] --> [batch_size, query_seq_len, hidden]
        attention_flow = self.cross_att_layer_norm(attention_flow) # Just normalization so shape will remain the same: [batch_size, query_seq_len, hidden] --> [batch_size, query_seq_len, hidden]

        fcn_flow = self.fcn(attention_flow) # [batch_size, query_seq_len, hidden]
        fcn_flow = self.dropout(fcn_flow) # Drop connect (since it is a dense network with lot of parameters): Shape will remain the same [batch_size, query_seq_len, hidden]
        fcn_flow = fcn_flow + attention_flow # Skip connection (residual): Shape remains the same [batch_size, query_seq_len, hidden]
        x = self.fcn_layer_norm(fcn_flow) # Normalization will not change shape: [batch_size, query_seq_len, hidden]
...
class TransfomerEncoderDecoderModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encoder_embeddings = nn.Embedding(num_embeddings=self.src_vocab_size, embedding_dim=hidden)
        self.decoder_embeddings = nn.Embedding(num_embeddings=self.tgt_vocab_size, embedding_dim=hidden)
        self.positional_emb = nn.Embedding(num_embeddings=self.src_vocab_size, embedding_dim=hidden)

        self.out_proj = nn.Linear(in_features=hidden, out_features=self.tgt_vocab_size)
        ...
        self.encoder_layers = nn.ModuleList([TransformerEncoderLayer(hidden=self.hidden, num_heads=self.num_heads, fcn_hidden=self.fcn_hidden, dropout=self.dropout_rate, causal=False) for _ in range(self.num_layers)])
        self.decoder_layers = nn.ModuleList([TransformerDecoderLayer(hidden=self.hidden, num_heads=self.num_heads, fcn_hidden=self.fcn_hidden, dropout=self.dropout_rate) for _ in range(self.num_layers)])
        # YOUR CODE ENDS HERE
    ...
    def _encode(self, input_ids, key_padding_mask):
        ...
        # NOTE: Code taken from previous homework
        encoder_emb = self.encoder_embeddings(input_ids) # [batch_size, src_seq_len, hidden]
        encoder_hidden_states = self._add_positions(encoder_emb) # Should not disturb the embeddings: [batch_size, src_seq_len, hidden]
        for encoder_layer in self.encoder_layers:
            encoder_hidden_states = encoder_layer(x=encoder_hidden_states, key_padding_mask=key_padding_mask) # [batch_size, src_seq_len, hidden] Because we are just stacking up layers which won't disturb the size of embedding of input sequence
        # YOUR CODE ENDS HERE
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        # YOUR CODE STARTS HERE
        # NOTE: Code taken from previous homework
        decoder_emb = self.decoder_embeddings(decoder_input_ids) # [batch_size, out_seq_len, hidden]
        decoder_hidden_states = self._add_positions(decoder_emb) # Should not disturb the embeddings: [batch_size, out_seq_len, hidden]
        for decoder_layer in self.decoder_layers:
            decoder_hidden_states = decoder_layer(decoder_hidden_states=decoder_hidden_states, encoder_hidden_states=encoder_hidden_states, key_padding_mask=key_padding_mask) # [batch_size, out_seq_len, hidden] --> [batch_size, out_seq_len, hidden]
