...
def preprocess_function(
    ...
    # This is because, in generation task, such as seq2seq generation task, to initiate the 
    # decoding process, we can add a starter token (BOS) to generate sequence which will be of max
    # size 1 less than the provided maximum size by the user. 
    # END OF YOUR ANSWER
    decoder_input_ids = []
    labels = []
    ...
    # Because in sequence to sequence generation task, such as translation, to trigger the 
    # translation process, we can give an artificial tag such as BOS token to generate next tokens. 
    # This token is used by decoder as input to generate next word (first word). 
    # From here onwards, the decoder can start using the generated words to generate next words.
    # END OF YOUR ANSWER
    ...
    *,
    ...
    for batch in tqdm(dataloader, desc="Evaluation"):
        with torch.inference_mode():
            ...
            # YOUR ANSWER HERE (please limit your answer to 1-2 sentences):
            # In forward we provide decoder input IDs as well, but in generation, we only provide input IDs (masked)
            # and expect the model to carry on with the generation of any length (within max length) as
            # translations might differ in number of words used from language to language. Generation ends at EOS token.
...
def main():
    ...
    src_tokenizer_path = os.path.join(args.output_dir, "english_tokenizer")
    # tgt_tokenizer_path = os.path.join(args.output_dir, f"{args.target_lang}_tokenizer")
    tgt_tokenizer_path = os.path.join(args.output_dir, "sql_tokenizer")
    ...
    # target_tokenizer = RobertaTokenizer.from_pretrained('Salesforce/codet5-small')
    target_tokenizer = RobertaTokenizer.from_pretrained('Salesforce/codet5-base')
    # YOUR CODE ENDS HERE
    ...
    # (readability matters)
    # YOUR CODE STARTS HERE
    # NOTE: Code taken from `train.py` in homework 4. This code was not part of expected work. 
